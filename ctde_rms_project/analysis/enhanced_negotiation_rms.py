"""
Enhanced Negotiation-Based RMS Scheduler with Multi-Round Bidding
...
class Job:
    def __init__(self, jid, family, subfamily, ptime, deadline, priority):
        ...
        self.urgency = 0.0  # calculated based on deadline proximity
        self.assigned = False
...
class Machine:
    def __init__(self, mid, base_failure=0.1, base_speed=1.0, flexibility=0.6):
        ...
        self.negotiation_strategy = random.choice(['aggressive', 'conservative', 'balanced'])
        self.reputation = 1.0  # 0-1, affects bid acceptance
        self.bid_history = []
...
class NegotiationProtocol:
    ...
    def __init__(self, max_rounds=5):  # INCREASED from 3 to 5
        self.max_rounds = max_rounds
        self.negotiation_log = []
        self.coalition_cache = {}  # For machine coalitions
        self.market_conditions = {'competition_level': 0.5}  # Track system-wide dynamics
    
    def conduct_negotiation(self, job: Job, machines: List[Machine], 
                          current_time: float, env) -> Tuple[int, Dict]:
        """
        Multi-round negotiation protocol:
        1. Job announces requirements
        2. Machines submit initial bids
        3. Job evaluates and may request counter-offers
        4. Machines adjust bids (up to max_rounds)
        5. Job selects winner or rejects all
        """
        ...
        if not operational:
            return -1, {'reason': 'no_operational_machines', 'rounds': 0}
        ...
        for round_num in range(self.max_rounds):
            ...
            # Check if job accepts or continues negotiation
            ...
            # Reject weak bids for next round - MORE AGGRESSIVE FILTERING
            rejection_threshold = acceptance_threshold * 0.85  # INCREASED from 0.7
            operational = [
            ...
            # Add competitive pressure: machines see each other's bids
            if len(operational) > 1:
                avg_bid = np.mean([b['score'] for b in bids])
                for m in operational:
                    m.bid_history.append(avg_bid)  # Learn from competition
    ...
    def _calculate_bid(self, job: Job, machine: Machine, 
        ...
        components['capability'] = 1.0 - (setup_time / 10.0)  # normalize
        
        # Workload component (0-1)
        avg_util = machine.utilization_time / max(1.0, current_time)
        components['workload'] = max(0.0, 1.0 - avg_util)
        
        # Speed/efficiency component
        components['speed'] = machine.speed / 1.3  # normalize to max speed
    ...
    def _apply_strategy(self, base_score: float, strategy: str, round_num: int) -> float:
        """Apply negotiation strategy to modify bid score with learning"""
        if strategy == 'aggressive':
            # Bid higher early, increase aggressively in later rounds
            return base_score * (1.15 + 0.08 * round_num)
        elif strategy == 'conservative':
            # Bid lower early, increase significantly if competition heats up
    ...
    def _get_acceptance_threshold(self, job: Job, round_num: int, max_rounds: int) -> float:
        ...
        base_threshold = 8.0  # INCREASED from 5.0 to make acceptance harder
        
        # Priority impact: high-priority jobs are more selective
        priority_factor = 0.8 + (job.priority * 0.4)
        
        # Urgency impact: urgent jobs become less selective over rounds
        urgency_discount = job.urgency * 0.15 * round_num
        
        if job.negotiation_strategy == 'aggressive':
            # Very high standards initially, decrease slower
            threshold = base_threshold * priority_factor * (1.5 - 0.15 * round_num)
        elif job.negotiation_strategy == 'conservative':
            # Moderate standards, willing to compromise
...
class RMS_Enhanced_Negotiation_Env:
    def __init__(self, num_jobs=10, num_machines=4, families=('A','B','C'),
        ...
        self.negotiation_protocol = NegotiationProtocol(max_rounds=3)
        self.reset()
    ...
    def reset(self):
        ...
        # Generate machines with diverse characteristics
    ...
    def assign_job_via_negotiation(self, job_id: int):
        """Assign job through enhanced negotiation protocol"""
        ...
        if fail:
            ...
            # ENHANCED negotiation bonus structure
            negotiation_bonus = 0
            if job.negotiation_rounds == 1:
                negotiation_bonus = -1.0  # Penalty for accepting too quickly
            elif job.negotiation_rounds == 2:
                negotiation_bonus = 1.5   # Optimal
            elif job.negotiation_rounds == 3:
                negotiation_bonus = 2.5   # Good thorough search
            elif job.negotiation_rounds == 4:
                negotiation_bonus = 2.0   # Slightly too long
            else:
                negotiation_bonus = 0.5   # Too many rounds
            
            # Priority-based negotiation expectations
            if job.priority > 0.7 and job.negotiation_rounds < 2:
                negotiation_bonus -= 2.0  # High-priority jobs should negotiate more
            
            reward = (job.priority * 10.0  # INCREASED from 8.0
                     - 0.25 * setup 
                     - 0.2 * tardiness 
                     + negotiation_bonus
                     + machine.reputation * 3.0)  # INCREASED reputation impact
    ...
    def get_state(self):
        ...
        for j in self.jobs:
            ...
            job_part.extend([
                ...
                urgency,  # NEW
                j.negotiation_rounds / 5.0,  # UPDATED max rounds to 5
                1.0 if j.negotiation_strategy == 'aggressive' else 0.0,  # NEW
                1.0 if j.negotiation_strategy == 'conservative' else 0.0  # NEW
            ])
...
def train_enhanced_negotiation(env, episodes=100, max_steps=400):
    ...
    # Simple DQN agent (reuse your existing CentralDQN)
    from dqn_agent import CentralDQN
